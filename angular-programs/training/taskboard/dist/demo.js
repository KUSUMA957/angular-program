...
let res1 = board.createTask(t1);
if (!res1.ok)
    throw new Error(res1.error);
board = res1.value.board;
let res2 = board.createTask(t2);
if (!res2.ok)
    throw new Error(res2.error);
board = res2.value.board;
let res3 = board.createTask(t3);
if (!res3.ok)
    throw new Error(res3.error);
board = res3.value.board;
let res4 = board.createTask(t4);
if (!res4.ok)
    throw new Error(res4.error);
board = res4.value.board;
...
let m1 = board.moveTask(task1.id, "IN_PROGRESS");
if (!m1.ok)
    throw new Error(m1.error);
board = m1.value.board;
let m2 = board.moveTask(task2.id, "IN_PROGRESS");
if (!m2.ok)
    throw new Error(m2.error);
board = m2.value.board;
...
let m3 = board.moveTask(task1.id, "REVIEW");
if (!m3.ok)
    throw new Error(m3.error);
board = m3.value.board;
let m4 = board.moveTask(task2.id, "REVIEW");
if (!m4.ok)
    throw new Error(m4.error);
board = m4.value.board;
// Attempt invalid move: BACKLOG → DONE for t3 (should fail)
const invalid = board.moveTask(task3.id, "DONE");
if (!invalid.ok)
    log("Invalid transition (expected)", invalid.error);
// Finish t1 (REVIEW → DONE)
let m5 = board.moveTask(task1.id, "DONE");
if (!m5.ok)
    throw new Error(m5.error);
board = m5.value.board;
// Search examples
...
// Metrics
log("Sprint total story points", sprintMetrics_1.SprintMetrics.totalStoryPoints(board, sprint));
log("Sprint burndown", sprintMetrics_1.SprintMetrics.burndown(board, sprint));
log("Sprint average cycle time (days)", sprintMetrics_1.SprintMetrics.averageCycleTime(board, sprint));
log("Current WIP count (instant)", sprintMetrics_1.SprintMetrics.wipCount(board));
// Batch move example using rest params: move t2 and t3 to REVIEW
const bm = board.moveTasks("REVIEW", task2.id, task3.id);
if (!bm.ok)
    throw new Error(bm.error);
